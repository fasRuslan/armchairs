<template>
  <section class="order">
    <div class="wrapper" v-if="cart_data.length">
      <h1 class="title mb-5">Оформление заказа</h1>
      <div class="error" v-if="validation.hasError() === true">
        Проверьте правильность заполнения полей!
      </div>
      <form class="order__form" @submit.prevent="submit">
        <h3 class="order__form-title">Ваш заказ</h3>
        ...
        <h3 class="order__form-title">Контактная информация</h3>
        <div class="form-group">
          <input
            class="form-control"
            type="text"
        ...
        <h3 class="order__form-title">Доставка</h3>
        <div class="form-group">
          <input
            type="text"
            placeholder="Индекс"
  ...
  validators: {
    name: function (value) {
      return Validator.value(value)
        ...
        .minLength(5, "Введите не менее 5 символов");
    },
    phone: function (value) {
      return Validator.value(value)
        .required("Поле обязательно для заполнения")
        .length(18, "Поле телефон должно содержать 10 цифр");
    },
    ...
    ind: function (value) {
      return Validator.value(value).digit("Только цифры");
    },
    city: function (value) {
